...
import { 
  ...
  MT4ConnectionConfig,
  MT4ImportResult 
} from '@/lib/mt4Integration';

interface MT4ConnectionPanelProps {
  onImportComplete?: () => void;
}

const MT4ConnectionPanel: React.FC<MT4ConnectionPanelProps> = ({ onImportComplete }) => {
  const [isConnected, setIsConnected] = useState(false);
  const [isConnecting, setIsConnecting] = useState(false);
  const [isImporting, setIsImporting] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const [accountInfo, setAccountInfo] = useState<MT4AccountInfo | null>(null);
  const [positions, setPositions] = useState<MT4Position[]>([]);
  const [tradeHistory, setTradeHistory] = useState<MT4Deal[]>([]);
  const [importResult, setImportResult] = useState<MT4ImportResult | null>(null);
  const [dateRange, setDateRange] = useState<'7d' | '30d' | '90d' | 'custom'>('7d');
  const [customFromDate, setCustomFromDate] = useState('');
  const [customToDate, setCustomToDate] = useState('');

  const [connectionConfig, setConnectionConfig] = useState<MT4ConnectionConfig>({
    accountId: 0,
    serverName: '',
    password: '',
    isReadOnly: true
  ...
  useEffect(() => {
    setIsConnected(mt4Service.getConnectionStatus());
    if (isConnected) {
      loadMT4Data();
    }
  }, []);
  ...
  const handleConnect = async () => {
    ...
    try {
      const success = await mt4Service.connect(connectionConfig);
      setIsConnected(success);
      if (success) {
        await loadMT4Data();
        toast.success('Connected to MetaTrader4 successfully');
      }
    } catch (error) {
      console.error('Connection error:', error);
      toast.error('Failed to connect to MetaTrader4');
    } finally {
  ...
  const loadMT4Data = async () => {
    if (!isConnected) return;
    try {
      const account = await mt4Service.getAccountInfo();
      setAccountInfo(account);
      const pos = await mt4Service.getPositions();
      setPositions(pos);
      const fromDate = new Date();
      fromDate.setDate(fromDate.getDate() - 7);
      const history = await mt4Service.getTradeHistory(fromDate);
      setTradeHistory(history);
    } catch (error) {
      console.error('Error loading MT4 data:', error);
    }
  };
  ...
  const handleImportTrades = async () => {
    ...
    try {
      ...
      const result = await mt4Service.importTrades(fromDate, toDate);
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">Connected to MetaTrader4</span>
                </div>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Button variant="outline" onClick={loadMT4Data}>
